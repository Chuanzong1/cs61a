...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    res=[i for i in paragraphs if select(i)]
    if k>=len(res):
        return ''
    return res[k]
    # END PROBLEM 1
...
def about(keywords):
    ...
    def select(paragraphs):
        if len(keywords)==0:
            return True
        for i in keywords:
            if i in split(lower(remove_punctuation(paragraphs))):
                return True
        return False
    return select
    # END PROBLEM 2
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in range(len(word_list)):
        if(typed_word==word_list[i]):
            return typed_word
        diff.append(diff_function(typed_word,word_list[i],limit))
    min_diff=min(diff)
    if(min_diff>limit):
        return typed_word
    return word_list[diff.index(min_diff)]'''
    ...
    return word_list[differences.index(min_diff)]
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    5   limit=3  '123456789'  'qwertyuio'
    """
    # BEGIN PROBLEM 6
    re=0
    def cal(typed,source,limit,re):
        if(re>limit): return re
        if(typed[0]!=source[0]):
            re+=1
        if(len(typed)<2 or len(source)<2):
            return re+abs(len(typed)-len(source))
        return cal(typed[1:],source[1:],limit,re)
    res=cal(typed,source,limit,0)
    return res
    # END PROBLEM 6
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0]==source[0]:
        return minimum_mewtations(typed[1:],source[1:],limit) # Feel free to remove or add additional cases
        # BEGIN
        "*** YOUR CODE HERE ***"
        # END
    else:
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(tpp)):
        ...
        for j in range(1,len(tpp[i])):
            every_time.append(tpp[i][j]-tpp[i][j-1])
        times.append(every_time)
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        faster_player=0
        faster_time=times[0][i]
        for j in range(1,len(times)):
            if(times[j][i]<faster_time):
                faster_time=times[j][i]
                faster_player=j
        result[faster_player].append(words[i])
    return result
    # END PROBLEM 10
